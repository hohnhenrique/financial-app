...
export function AccountsPage() {
  ...
  const createMut = useMutation({ mutationFn: accountsApi.create, onSuccess: () => { invalidate(); setForm(EMPTY) } })
  const updateMut = useMutation({ mutationFn: ({ id, data }: { id: string; data: AccountPayload }) => accountsApi.update(id, data), onSuccess: () => { invalidate(); setEditing(null); setForm(EMPTY) } })
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-[380px_1fr] gap-6">
        ...
        <div className="px-6 pb-6 space-y-5">
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm text-slate-400">Usada para identificar a conta.</span>
            </div>
          </div>
      ...
      <Card title="Contas Cadastradas">
        {(accounts ?? []).length === 0
          ? <div className="px-6 py-16 text-center text-slate-400 text-sm">Nenhuma conta cadastrada.</div>
          : <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="bg-slate-50 dark:bg-slate-700/50 border-b border-slate-100 dark:border-slate-700">
                  {['Conta','Tipo','Saldo','Status','Ações'].map(h => <th key={h} className="px-6 py-3 text-slate-500 dark:text-slate-400 font-medium text-left">{h}</th>)}
                </tr></thead>
                <tbody className="divide-y divide-slate-50 dark:divide-slate-700">
                  {(accounts ?? []).map(a => (
                    <tr key={a.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                      ...
                      <td className="px-6 py-4 font-semibold text-slate-700 dark:text-slate-200">{formatMoney(a.initial_balance_cents)}</td>
                      <td className="px-6 py-4">

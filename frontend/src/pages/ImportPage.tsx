...
export function ImportPage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (data.total === 0) { setError('Nenhuma transação encontrada.'); return }

      setRows(data.transactions.map(t => ({
        ...t,
        selected:    true,
        account_id:  defaultAccountId,
        category_id: '',           // usuário deve escolher por linha
      })))
      setStep('review')
    } catch (e: unknown) {
  ...
  const handleConfirm = async () => {
    ...
    if (missing.length > 0) {
      setError(`${missing.length} movimentação(ões) selecionada(s) sem categoria. Selecione ou desmarque-as.`)
      return
    }
  ...
  if (step === 'upload') return (
    <div className="max-w-2xl mx-auto space-y-5">
      <Card title="Importar Extrato" subtitle="Importe transações do seu banco em formato CSV.">
        <div className="px-8 py-6 space-y-5">
          {error && <Alert type="error" message={error} />}

          <Select label="Banco / Corretora" options={BANKS} value={bank} onChange={e => setBank(e.target.value)} required />

          <Select
            label="Conta padrão (pode alterar por linha depois)"
            options={accOptions}
          ...
          <div>
            ...
            >
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">
                Clique para selecionar <span className="text-blue-600 dark:text-blue-400 font-medium">.csv ou .txt</span>
              </p>
              <p id="file-name" className="text-xs text-slate-400 dark:text-slate-500 mt-1">Máximo 5MB</p>
              <input
                ref={fileRef}
                type="file"
                accept=".csv,.txt,.ofx"
          ...
          {/* Instruções */}
          <div className="bg-slate-50 dark:bg-slate-700/30 rounded-xl p-4 text-xs text-slate-500 dark:text-slate-400 space-y-1">
            {bank === 'rico'  && <><p className="font-semibold text-slate-600 dark:text-slate-300 mb-2">📋 Rico — Cartão de Crédito</p><p>Acesse o app ou site → Cartão → Fatura → Exportar → CSV</p></>}
            {bank === 'inter' && <><p className="font-semibold text-slate-600 dark:text-slate-300 mb-2">📋 Banco Inter</p><p>App → Extrato → Período → Exportar → CSV</p><p className="mt-1 text-slate-400">Ou: Internet Banking → Conta Corrente → Extrato → Exportar</p></>}
            {bank === 'nubank' && <><p className="font-semibold text-slate-600 dark:text-slate-300 mb-2">📋 Nubank</p><p>App → Perfil → Meus dados → Exportar dados → Transações CSV</p></>}
            {bank === 'itau' && <><p className="font-semibold text-slate-600 dark:text-slate-300 mb-2">📋 Itaú</p><p>Internet Banking → Conta → Extrato → Salvar → CSV</p></>}
            {bank === 'bradesco' && <><p className="font-semibold text-slate-600 dark:text-slate-300 mb-2">📋 Bradesco</p><p>Internet Banking → Extrato → Exportar CSV</p></>}
            {bank === 'generic' && <><p className="font-semibold text-slate-600 dark:text-slate-300 mb-2">📋 Formato genérico</p><p>Colunas: Data (DD/MM/AAAA), Descrição, Valor (negativo = despesa)</p><p>Separador: ponto-e-vírgula ou vírgula</p></>}
          </div>
  ...
  if (step === 'review') return (
    <div className="space-y-5">
      ...
      {/* Ações em massa */}
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 px-5 py-3 flex flex-wrap gap-3 items-center shadow-sm">
        <span className="text-xs font-medium text-slate-500 dark:text-slate-400">Ações em massa:</span>
        <button onClick={selectAll}   className="text-xs text-blue-600 dark:text-blue-400 hover:underline">Selecionar todas</button>
        <button onClick={deselectAll} className="text-xs text-slate-500 dark:text-slate-400 hover:underline">Desmarcar todas</button>
        <div className="h-4 w-px bg-slate-200 dark:bg-slate-600" />
        <span className="text-xs font-medium text-slate-500 dark:text-slate-400">Categoria padrão:</span>
        <div className="w-52">
          <select
            onChange={e => { if (e.target.value) setCatAll(e.target.value) }}
            defaultValue=""
            className="w-full px-3 py-1.5 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs focus:outline-none focus:ring-1 focus:ring-blue-400"
          >
          ...
          <table className="w-full text-sm">
            <thead className="sticky top-0 z-10">
              <tr className="bg-slate-50 dark:bg-slate-700/90 border-b border-slate-100 dark:border-slate-700">
                ...
                <th className="px-4 py-3 text-left text-slate-500 dark:text-slate-400 font-medium whitespace-nowrap">Data</th>
                <th className="px-4 py-3 text-left text-slate-500 dark:text-slate-400 font-medium">Descrição</th>
                <th className="px-4 py-3 text-left text-slate-500 dark:text-slate-400 font-medium">Tipo</th>
                <th className="px-4 py-3 text-right text-slate-500 dark:text-slate-400 font-medium">Valor</th>
                <th className="px-4 py-3 text-left text-slate-500 dark:text-slate-400 font-medium w-48">Conta</th>
      ...
      <Card>
        <div className="px-8 py-12 text-center space-y-5">
          ...
          <div className="flex gap-3">
            <Button variant="secondary" onClick={() => { setStep('upload'); setRows([]); setResult(null) }} className="flex-1">
              Nova importação
            </Button>

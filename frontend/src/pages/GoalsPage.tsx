...
export function GoalsPage() {
  ...
  const [sortBy, setSortBy]   = useState('year_month')
  ...
  const sorted = [...(allGoals ?? [])].sort((a, b) => {
    const va = sortBy === 'income_goal'  ? a.income_goal
             : sortBy === 'expense_goal' ? a.expense_goal
             : a.year_month
    const vb = sortBy === 'income_goal'  ? b.income_goal
             : sortBy === 'expense_goal' ? b.expense_goal
             : b.year_month
  ...
  const previewIncome  = Math.round(parseFloat((incomeGoal  || '0').replace(',', '.')) * 100)
  const previewExpense = Math.round(parseFloat((expenseGoal || '0').replace(',', '.')) * 100)
  const previewBalance = previewIncome - previewExpense
  const showPreview    = incomeGoal !== '' || expenseGoal !== ''
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-[420px_1fr] gap-6">
          ...
          <div className="space-y-5">
            ...
            {showPreview && (
              <div className="bg-slate-50 dark:bg-slate-700/30 rounded-xl p-4 text-sm space-y-2">
                <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Prévia</p>
                {[
                  ['Receita prevista',  formatMoney(previewIncome),  'text-emerald-600 dark:text-emerald-400'],
                  ['Limite de despesa', formatMoney(previewExpense), 'text-red-500 dark:text-red-400'],
                ].map(([l, v, c]) => (
                ...
                <div className="flex justify-between border-t border-slate-200 dark:border-slate-600 pt-2 mt-1">
                  ...
                  <span className={`font-bold ${previewBalance >= 0 ? 'text-blue-600 dark:text-blue-400' : 'text-red-500'}`}>
      ...
      <div className="space-y-5">
          ...
          <div className="px-6 pb-6">
            {isLoading
              ? <div className="flex items-center justify-center h-32">
                  <div className="animate-spin w-6 h-6 border-4 border-[#1B4F8A] border-t-transparent rounded-full" />
                </div>
              : !hasGoal
                ? <div className="py-10 text-center text-slate-400 dark:text-slate-500 text-sm">
                    Nenhuma meta definida para {monthLabel(selectedMonth)}.
                  </div>
                : <div className="pt-2 space-y-5">
                    ...
                    {status!.expense_pct !== null && status!.expense_pct >= 80 && status!.expense_pct < 100 && (
                      <Alert type="warning" message={`Atenção: ${status!.expense_pct}% do limite de despesas utilizado.`} />
                    )}
        ...
        {/* Listagem de metas com paginação */}
        <Card title={`Metas cadastradas ${total ? `(${total})` : ''}`}>
          {paged.length === 0
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-slate-50 dark:bg-slate-700/50 border-b border-slate-100 dark:border-slate-700">
                        <SortHeader label="Mês"           field="year_month"   current={sortBy} direction={sortDir} onChange={handleSort} />
                        <SortHeader label="Meta receita"  field="income_goal"  current={sortBy} direction={sortDir} onChange={handleSort} />
                        <SortHeader label="Limite despesa" field="expense_goal" current={sortBy} direction={sortDir} onChange={handleSort} />
                        <th className="px-6 py-3 text-slate-500 dark:text-slate-400 font-medium text-left">Saldo previsto</th>
                      ...
                      {paged.map(g => (
                        <tr key={g.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                          <td className="px-6 py-4">
                            <button
                              onClick={() => loadGoal(g.year_month)}
                              className="font-medium text-slate-700 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400 transition-colors"
                            >
                          ...
                          <td className="px-6 py-4">
                            <MoneyValue
                              ...
                              className={`font-semibold ${g.income_goal - g.expense_goal >= 0 ? 'text-blue-600 dark:text-blue-400' : 'text-red-500'}`}
                            />
                          </td>
